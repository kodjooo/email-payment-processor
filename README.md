...
все взаимодействие с удаленной БД происходит через DBeaver, для этого через терминал нужно установить соединение с удаленным сервером:
ssh -i ~/.ssh/id_rsa_pem -L 5433:localhost:5432 developer@dev.racefinder.pt
